...
int main(int argc, char* argv[])
{
    ...
    uint32_t **data = (uint32_t **)0x12345678;
    data = setup_data(input, INPUT_WIDTH, INPUT_HEIGHT);
    std::cout << "[main] data addr: " << data << "\n";

    uint32_t * histo = (uint32_t *)0x87654321;
    histo = setup_histo();
    //std::cout << "[main] histo addr: " << (uint *)histo << "\n";

    /* This is the call
     * you will use to time your parallel implementation */
    TIME_IT("opt_2dhisto",
            1000,
            opt_2dhisto(histo, data, INPUT_HEIGHT, INPUT_WIDTH);)

    /* Include your teardown code below (temporary variables, function calls, etc.) */

    std::cout << "[main] kernel_bins addr: " << (uint *)kernel_bins << "\n";
    uint32_t *histo_bins = (uint32_t *)malloc(HISTO_HEIGHT * HISTO_WIDTH * sizeof(uint32_t));
    copy_final(data, histo_bins, histo);

    for(size_t i = 0; i < HISTO_HEIGHT * HISTO_WIDTH; ++i) {
      if (histo_bins[i] > UINT8_MAX) {
        kernel_bins[i] = UINT8_MAX;
      } else {
        kernel_bins[i] = (uint8_t)histo_bins[i];
      }
    }

    //for(int i = 0; i < HISTO_WIDTH * HISTO_HEIGHT; ++i)
    //  std::cout << (int)kernel_bins[i] << " ";
