...
int main(int argc, char* argv[])
{
    ...
    uint32_t *newinput=newdata_collect(input, INPUT_HEIGHT, INPUT_WIDTH);
    ...
    //Mallc in data on device
    uint32_t *  data = setup(newinput, INPUT_WIDTH, INPUT_HEIGHT);
    
    //Mallc in histo on device
    uint32_t *  histo = setuphisto();
    ...
    uint32_t *kernelbins = (uint32_t*)malloc(HISTO_HEIGHT*HISTO_WIDTH*sizeof(uint32_t));
    copy_final(data, kernelbins, histo);
    

    // transfer from uint32_t to uint8_t
    for(int i = 0; i < HISTO_HEIGHT * HISTO_WIDTH; i++){
      if(kernelbins[i] > 255)
        kernel_bins[i] = 255;
      else
	kernel_bins[i] = kernelbins[i];
    }    
